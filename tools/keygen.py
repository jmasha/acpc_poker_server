...
def sendEmail(recipient,key,resend):
    ...
    '''
    URL = 'http://www.surveymonkey.com/s/Poker01'
    admin_list = 'gamgrp@ucalgary.ca'

    # Create the container (outer) email message.
    msg = MIMEMultipart()
    msg['Subject'] = 'Successful registration'
    msg['From'] = 'Poker Study Registration <dontemailpolaris@mailinator.com>'
    msg['To'] = recipient
...
<html>
 <head></head>
 <body>
  <p>Welcome to the Poker Play Among University Students Study.<br>
  Your password key is : <b>'''+key+'''</b><br><br>
  This study contains three components:<br>
  1.) A brief survey about gambling<br>
  2.) An online poker game of texas hold'em<br>
  3.) Ten post play questions<br>
  ...
  '''+URL+'''
    ...
    '''
    text = '''
Welcome to the Poker Play Among University Students Study.
Your password key is '''+key+'''

This study has three components: 
1.) A brief survey about gambling 
2.) An online poker game of texas hold'em 
3.) Ten post-play questions

You will require your password key to do all components. 
...
Please proceed to the following URL to begin: '''+URL
    ...
    if not resend:
        ...
        reg_msg['From'] = 'Poker Study Registration <dontemailpolaris@mailinator.com>'
...
def getNextConfig():
    ...
    fp.close()
    if config_option == 'W':
        return "psyctest.weak.limit.config.xml"
    else:
        return "psyctest.strong.limit.config.xml"
...
def usage():
    ...
    --config=     Specify a config file.  The default is headsup.limit.config
    --no-key      Use the username as the key value
    --test        Test the existance of a duplicate only.  Returns SUCCESS or DUPLICATE
...
def main(argv):
    ...
    salt = ""
    seed_salt = ""
    config = "headsup.limit.config.xml"
    benchmark = False
    num_users = ""
    no_key = False
    verbose = False
